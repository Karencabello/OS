...
sem_t buff2Free;
sem_t buff2Filled;
sem_t producer_exit_sem; // Nuevo semáforo para la terminación del productor
...
void* producer(void* arg) {
    ...
    while (1) {
        ...
        sem_wait(&producer_exit_sem); // Espera para verificar la terminación
        if (done) {
            sem_post(&producer_exit_sem); // Libera para el siguiente productor
        ...
        if (strcmp(n, "EXIT") == 0) { // Usuario escribe EXIT, terminar
            ...
            // Liberar buff1Free para todos los productores bloqueados
            for (int i = 0; i < SIZE; i++) {
                sem_post(&buff1Free);
            }
            for (int i = 0; i < NUM_TH_PROD; i++) {
                sem_post(&producer_exit_sem); // Libera todos los productores
            }

            for (int i = 0; i < NUM_TH_PC + NUM_TH_CONS; i++) {
                sem_post(&buff1Filled); // Desbloquea cons_prod
...
int main(int argc, char* argv[]) {
    ...
    sem_init(&producer_exit_sem, 0, 1); // Inicializar semáforo de terminación
