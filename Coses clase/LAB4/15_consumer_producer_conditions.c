...
void * consumer(void*) {
    while(!finish) {
        ...
        nBufferWithData -= 1;
        pthread_cond_signal(&bufferFull); // The buffer has a new empty position, so wake up a producer if needed.
...
void * producer(void *) {
    while(!finish) {
        ...
        // While the condition is not met, I will wait, releasing the lock so that a
        // producer can work.
        while (nBufferWithData == N) {
        ...
        nBufferWithData += 1;
        pthread_cond_signal(&bufferEmpty); // The buffer has a new empty position, so wake up a producer if needed.
        pthread_mutex_unlock(&lock);
